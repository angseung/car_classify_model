...
class CarDataset(data.Dataset):
    def __init__(self, x_tensor: Tensor, y_tensor: Tensor, base_dir: str = "./data") -> None:
...
def reorder_train_data_dir(base_dir: str = "./data") -> None:
    class_labels = get_json_annotation(f"{base_dir}/train/labels/meta.json")["class_names"]
    class_dict = {index : name for index, name in enumerate(class_labels)}
    img_list = os.listdir(f"{base_dir}/train/images")
    assert len(img_list) == get_json_annotation(f"{base_dir}/train/labels/meta.json")["num_images"]
    ...
    for file_name in img_list:
        ...
        label = get_json_annotation(f"{base_dir}/train/labels/annotations/{json_file_name}")
...
def get_torch_dataloader(base_dir: str = "./data", target_dir: str = "./data_reordered", transform: torchvision.transforms = None) -> torch.utils.data.DataLoader:
